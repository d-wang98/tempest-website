        ...
        <Container>
          <FadeIn className="flex justify-center mb-10">
            <div className="inline-flex flex-wrap justify-center gap-1.5 rounded-2xl border border-gray-200 bg-white p-1.5 shadow-sm">
              ...
              <TabButton active={tab === "high-value"} onClick={() => setTab("high-value")} soon={t("soon")}>
                {t("tabHV")}
              </TabButton>
              <TabButton active={tab === "more"} onClick={() => setTab("more")} soon={t("soon")}>
                {t("tabMore")}
              </TabButton>
            </div>
          </FadeIn>
          ...
          {tab === "high-value" && (
            <ComingSoonTab title={t("hvTitle")} desc={t("hvDesc")} waitlist={t("joinWaitlist")} comingSoon={t("comingSoon")} />
          )}
          {tab === "more" && (
            <ComingSoonTab title={t("moreTitle")} desc={t("moreDesc")} waitlist={t("joinWaitlist")} comingSoon={t("comingSoon")} />
          )}
        </Container>
      </section>
    </>
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all ${
        active ? "bg-brand-primary text-white shadow-sm" : "text-gray-500 hover:text-gray-800"
      }`}
    >
      {children}
      {soon && (
...
function TradeFinanceTab() {
  ...
  const features = tfFeatureIcons.map((icon, i) => ({
    icon,
    title: t(`tfF${i + 1}title` as never),
    body: t(`tfF${i + 1}body` as never),
  }));

  const audiences = audienceEmojis.map((emoji, i) => ({
    emoji,
    label: t(`tfA${i + 1}label` as never),
    body: t(`tfA${i + 1}body` as never),
  }));

  const steps = [1, 2, 3, 4].map((n) => ({
    label: t(`tfS${n}label` as never),
    desc: t(`tfS${n}desc` as never),
  }));
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="grid lg:grid-cols-2">
            <div className="p-8 md:p-12">
              ...
              <h3 className="text-3xl font-black leading-tight md:text-4xl">
                {t("tfHeadline")}
                <br />
                <span className="text-brand-cyan">{t("tfSubheadline")}</span>
              </h3>
              <p className="mt-4 text-base leading-relaxed text-white/65">{t("tfDesc")}</p>
      ...
      <div>
        <FadeIn>
          <h4 className="text-base font-black text-gray-900 mb-5">{t("tfFeaturesHeading")}</h4>
        </FadeIn>
      ...
      <FadeIn>
        <div className="rounded-3xl border border-gray-100 bg-white p-8 shadow-sm">
          <h4 className="text-base font-black text-gray-900 mb-8 text-center">{t("tfStepsHeading")}</h4>
